...
const snowcrashLeaves = ["0x8ec27e9f5bab92b94b15ab4f1de164c5a5da2e99",
...
export default function Minter(props) {
    ...
    const snowcrashTree = new MerkleTree(snowcrashLeaves, keccak256, { sortPairs: true })

    const snowcrashProof = snowcrashTree.getHexProof(leaf)

    const { config: publicConfig } = usePrepareContractWrite({
        address: contractAddress,
        abi: props.contractAbi,
        functionName: "normieMint",
        args: [quantity],
        overrides: {
            value: ethers.utils.parseEther("0.0256").mul(quantity),
        },
    })
    const { config: snowcrashConfig, error: snowcrashError } = usePrepareContractWrite({
        address: contractAddress,
        abi: props.contractAbi,
        functionName: "snowcrashMint",
        args: [snowcrashProof],
        overrides: {
    ...
    const { write: snowcrashWrite } = useContractWrite(snowcrashConfig)
    if (isConnected) {
        return (
            <div className="container">
                <div className="generalSale">
                    <button className="mint_btn" disabled={!publicWrite} onClick={() => publicWrite?.()}>Mint</button>
                    <input
                        type="number"
                        id="quantity"
                        name="quantity"
                    ...
                    <label className="quant_label" for="quantity">3 Mint Max. 1 TX per wallet.</label>
                </div>
                <div className="break"></div>
                {
                    !snowcrashError &&
                    <button className="mint_btn" disabled={!snowcrashWrite} onClick={() => snowcrashWrite?.()}>Snowcrash Mint</button>
                }
            </div>
        )
    }
